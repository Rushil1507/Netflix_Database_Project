...
WITH new_user AS (
    INSERT INTO users (email, password_hash, full_name, phone_number, date_of_birth, country_code, account_status, email_verified)
    VALUES ('rahul.mehta@example.com', '$2b$12$examplehashvalue', 'Rahul Mehta', '9123456780', '1995-08-14', 'IN', 'active', true)
    RETURNING user_id
)
INSERT INTO profiles (user_id, profile_name, is_kids, is_primary)
SELECT user_id, 'Rahul', FALSE, TRUE
FROM new_user
...
VALUES ('Standard Plus', 11.99, 119.99, 'USD', 2, 3, 'HD', FALSE, FALSE, 3, FALSE)
...
COMMIT;
SELECT user_id, status, length(status) FROM subscriptions WHERE user_id = 1;
...
VALUES (1, 9, 'Episode 9', 'Episode 9 synopsis...', 45, '2024-09-08', 'https://cdn.example-streaming.com/video/e9...', 'https://cdn.example-streaming.com/thumb/e9...')
...
SELECT profile_id, content_id, COUNT(*)
FROM ratings
GROUP BY profile_id, content_id
HAVING COUNT(*) > 1;
ALTER TABLE ratings
ADD CONSTRAINT uq_ratings_profile_content UNIQUE (profile_id, content_id);
